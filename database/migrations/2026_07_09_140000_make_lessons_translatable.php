<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        if (! Schema::hasTable('lessons')) {
            return;
        }

        foreach (['title', 'description'] as $column) {
            if (! Schema::hasColumn('lessons', $column)) {
                continue;
            }

            foreach (DB::table('lessons')->get(['id', $column]) as $lesson) {
                $value = $lesson->{$column};
                $decoded = json_decode($value, true);

                if (is_array($decoded)) {
                    continue;
                }

                DB::table('lessons')->where('id', $lesson->id)->update([
                    $column => json_encode([
                        'en' => $value,
                        'ru' => $value,
                        'tg' => $value,
                    ]),
                ]);
            }

            if (Schema::getConnection()->getDriverName() === 'mysql') {
                $type = $column === 'title' ? 'JSON NOT NULL' : 'JSON NOT NULL';
                DB::statement("ALTER TABLE lessons MODIFY `{$column}` {$type}");
            }
        }
    }

    public function down(): void
    {
        if (! Schema::hasTable('lessons')) {
            return;
        }

        foreach (['title', 'description'] as $column) {
            if (! Schema::hasColumn('lessons', $column)) {
                continue;
            }

            if (Schema::getConnection()->getDriverName() === 'mysql') {
                $type = $column === 'title' ? 'VARCHAR(255) NOT NULL' : 'TEXT NOT NULL';
                DB::statement("ALTER TABLE lessons MODIFY `{$column}` {$type}");
            }

            foreach (DB::table('lessons')->get(['id', $column]) as $lesson) {
                $decoded = json_decode($lesson->{$column}, true);

                if (! is_array($decoded)) {
                    continue;
                }

                DB::table('lessons')->where('id', $lesson->id)->update([
                    $column => $decoded['en'] ?? reset($decoded) ?: '',
                ]);
            }
        }
    }
};
