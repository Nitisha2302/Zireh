...
<div class="tab-content">
    @foreach (['en' => __('admin.english'), 'ru' => __('admin.russian'), 'tg' => __('admin.tajik')] as $locale => $label)
        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $locale }}" role="tabpanel">
            <div class="mb-0">
                <label class="form-label" for="name-{{ $locale }}">{{ __('admin.category_name_in', ['language' => $label]) }}</label>
                <input id="name-{{ $locale }}" type="text"
                    class="form-control @error("name.$locale") is-invalid @enderror"
                    wire:model.blur="name.{{ $locale }}" placeholder="{{ __('admin.category_name_in', ['language' => $label]) }}">
                @error("name.$locale")
