<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <div>
            <h4 class="mb-1">{{ __('admin.customer_wallet') }}</h4>
            <p class="mb-0 text-body-secondary">{{ $customer->name }} · {{ $customer->phone }}</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.customers.edit', $customer) }}" class="btn btn-label-secondary">{{ __('admin.back_to_customer') }}</a>
            <a href="{{ route('admin.wallet-transactions.index', ['user' => $customer->id]) }}" class="btn btn-label-primary">{{ __('admin.all_transactions') }}</a>
        </div>
    </div>
    ...
    <div class="row g-4">
        <div class="col-lg-4">
            <div class="card h-100">
                <div class="card-body">
                    <p class="text-body-secondary mb-1">{{ __('admin.current_balance') }}</p>
                    ...
                    <ul class="nav nav-tabs card-header-tabs" role="tablist">
                        <li class="nav-item">
                            <button type="button" class="nav-link {{ $walletAction === 'add' ? 'active' : '' }}" wire:click="$set('walletAction', 'add')">
                                <i class="icon-base ti tabler-plus me-1"></i>{{ __('admin.add_funds') }}
                            </button>
                        </li>
                        <li class="nav-item">
                            <button type="button" class="nav-link {{ $walletAction === 'deduct' ? 'active' : '' }}" wire:click="$set('walletAction', 'deduct')">
                                <i class="icon-base ti tabler-minus me-1"></i>{{ __('admin.deduct_balance') }}
                            </button>
                        </li>
                    </ul>
                </div>
                <div class="card-body">
                    @if ($walletAction === 'add')
                        <form wire:submit="addFunds" class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label">{{ __('admin.amount_tjs') }}</label>
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" class="form-control @error('description') is-invalid @enderror" wire:model="description" placeholder="{{ __('admin.deposit_note_placeholder') }}">
                    ...
                    @else
                        <form wire:submit="confirmDeduct" class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <small class="text-body-secondary">{{ __('admin.available_balance', ['amount' => number_format((float) $wallet->balance, 2)]) }}</small>
                            </div>
                            <div class="col-md-8">
                                <label class="form-label">{{ __('admin.description') }}</label>
                                <input type="text" class="form-control @error('deductDescription') is-invalid @enderror" wire:model="deductDescription" placeholder="{{ __('admin.deduction_reason_placeholder') }}">
    ...
    <div class="card mt-4">
        <div class="card-header"><h5 class="mb-0">{{ __('admin.wallet_history') }}</h5></div>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($transaction->isRevertable())
                                    <button type="button" class="btn btn-sm btn-label-warning" wire:click="confirmRevert({{ $transaction->id }})">{{ __('admin.revert') }}</button>
                                @elseif ($transaction->reverts_transaction_id)
                                    <small class="text-body-secondary">{{ __('admin.revert_of', ['id' => $transaction->reverts_transaction_id]) }}</small>
                                @elseif ($transaction->reverted_by_transaction_id)
                                    <small class="text-body-secondary">{{ __('admin.reverted_by', ['id' => $transaction->reverted_by_transaction_id]) }}</small>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="text-center py-4 text-body-secondary">{{ __('admin.no_wallet_transactions') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
