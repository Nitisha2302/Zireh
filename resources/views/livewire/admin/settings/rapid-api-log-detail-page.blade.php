<div class="container-xxl flex-grow-1 container-p-y">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    <div>
      <h4 class="mb-1">{{ __('admin.rapid_api_log_detail') }} #{{ $log->id }}</h4>
      <p class="mb-0 text-body-secondary">{{ $log->created_at?->format('d M Y H:i:s') }}</p>
    </div>
    <a href="{{ route('admin.settings.rapid-api-logs.index') }}" class="btn btn-label-secondary">
  ...
  <div class="row g-4">
    <div class="col-lg-4">
      <div class="card h-100">
        ...
        <div class="card-body">
          <dl class="row mb-0">
            <dt class="col-sm-5">{{ __('admin.rapid_api_log_method') }}</dt>
            <dd class="col-sm-7"><span class="badge bg-label-info">{{ $log->method }}</span></dd>

            <dt class="col-sm-5">{{ __('admin.rapid_api_log_endpoint') }}</dt>
            <dd class="col-sm-7 font-monospace small">{{ $log->endpoint }}</dd>

            <dt class="col-sm-5">{{ __('admin.rapid_api_log_source') }}</dt>
            ...
            <dt class="col-sm-5">{{ __('admin.rapid_api_log_duration') }}</dt>
            ...
            @if ($log->response_truncated)
              ...
              <dd class="col-sm-7 text-warning">{{ __('admin.rapid_api_log_response_truncated') }}</dd>
            @endif
          </dl>
        </div>
      </div>
    ...
    <div class="col-lg-8">
      <div class="card mb-4">
        <div class="card-header">
          <h5 class="mb-0">{{ __('admin.rapid_api_log_request') }}</h5>
        </div>
        <div class="card-body">
          ...
          @else
            <p class="mb-0 text-body-secondary">{{ __('admin.rapid_api_log_no_request_body') }}</p>
          @endif
        </div>
      </div>
      ...
      <div class="card">
        <div class="card-header">
          <h5 class="mb-0">{{ __('admin.rapid_api_log_response') }}</h5>
        </div>
        <div class="card-body">
          ...
          @else
            <p class="mb-0 text-body-secondary">{{ __('admin.rapid_api_log_no_response_body') }}</p>
          @endif
        </div>
      </div>
    </div>
