...
class Platform1688CartService
{
    ...
    public function add(User $user, array $data): array
    {
        ...
        $this->assertQuantity($quantity, $detail);
    ...
    {
        try {
            $detail = $this->alibaba1688Service->find($productId, $lang);
        } catch (ElimException) {
        ...
        if (empty($detail['id']) || ($detail['platform'] ?? null) !== UserCartItem::PLATFORM_1688) {
            throw ValidationException::withMessages([
    ...
    protected function assertQuantity(int $quantity, array $detail): void
    {
        $moq = max(1, (int) ($detail['minimum_order_quantity'] ?? 1));

        if ($quantity < $moq) {
            throw ValidationException::withMessages([
                'quantity' => [__('api.cart_moq_not_met', ['moq' => $moq])],
            ]);
        }
    }
    ...
    {
        if ($item->user_id !== $user->id || $item->platform !== UserCartItem::PLATFORM_1688) {
            throw ValidationException::withMessages([
